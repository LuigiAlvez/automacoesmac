...
def reinstall_slack():
    ...
    if ok and sudo_password:
        try:
            # Comandos para desinstalar e reinstalar o Slack
            uninstall_command = 'sudo rm -rf /Applications/Slack.app'
            install_command = 'brew install --cask slack'
            os.system(f'echo {sudo_password} | sudo -S {uninstall_command}')
            os.system(f'echo {sudo_password} | sudo -S {install_command}')
            custom_messagebox("Sucesso", "Slack reinstalado com sucesso.", "logo-ifood-512.png")
        except Exception as e:
            custom_messagebox("Erro", f"Erro ao reinstalar o Slack: {e}", "logo-ifood-512.png")
    else:
        custom_messagebox("Aviso", "Senha não fornecida. Ação cancelada.", "logo-ifood-512.png")
# Front End
...
def create_gui():
    app = QApplication([])
    window = QWidget()
    window.setWindowTitle("iTech Auto Suporte")
    window.setFixedSize(1140, 570)
    window.setStyleSheet("""
        QWidget { background-color: #F5F5F5; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        QGroupBox::title { font-size: 25px; color: #D32F2F; }
    ...
    app_buttons = [
        ("Reinstalar Google Chrome", uninstall_google_chrome, "google-symbol_2875331.png"),
        ("Reparar VPN", install_forticlient_vpn, "vpn_5322131.png"),
        ("Reparar Slack", reinstall_slack, "slack-logo-50A32CEFAA-seeklogo.com.png")  # Adicionando botão para reinstalar o Slack
    ]
